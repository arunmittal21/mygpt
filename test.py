...
def load_model(checkpoint_path="checkpoint/ckpt.pt"):
    # checkpoint_path = os.path.join(out_dir, file)
    if os.path.exists(checkpoint_path):
        ...
        encode, decode = get_encoder_decoder(
            checkpoint["str_to_int"], checkpoint["int_to_str"]
        )
        return model, context_length, encode, decode
    else:
...
def generate(
    ...
    for i in range(new_chars):
        ...
        probabilities = torch.nn.functional.softmax(
            last_time_step / temperature, dim=-1
        )
        ...
        next_char_decoded = decode(
            int(next_char.item())
        )  # int_to_str.get(next_char.item(), " ")
...
def test(file, generate_len=300):
    ...
    model, context_length, encode, decode = load_model(file)
